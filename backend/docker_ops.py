...
class DockerOperations:
    def __init__(self):
        self.compose_base_path = os.getenv(
            "TRANSDOCK_COMPOSE_BASE", "/mnt/cache/compose")
        self.appdata_base_path = os.getenv(
    ...
    async def parse_compose_file(self, compose_file_path: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Failed to parse compose file {compose_file_path}: {e}")
            raise

    async def extract_volume_mounts(
            self, compose_data: Dict) -> List[VolumeMount]:
                ...
                if isinstance(volume, str):
                    ...
                    if ':' in volume:
                        ...
                        if len(parts) >= 2:
                            ...
                            # Skip bind mounts that aren't in our data
                            # directories
    ...
    async def start_compose_stack_remote(
            self,
            target_host: str,
            compose_dir: str,
            ssh_user: str = "root",
        ...
        logger.info(
            f"Starting compose stack in {compose_dir} on {target_host}")
        ...
        if returncode != 0 or not stdout.strip():
            logger.error(
                f"No compose file found in {compose_dir} on {target_host}")
            return False
        ...
        if returncode != 0:
            logger.error(
                f"Failed to start compose stack on {target_host}: {stderr}")
            return False

        logger.info(
            f"Successfully started compose stack in {compose_dir} on {target_host}")
        return True

    async def update_compose_file_paths(
            self, compose_file_path: str, volume_mapping: Dict[str, str]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to update compose file {compose_file_path}: {e}")
            return False

    async def create_target_compose_dir(
            self,
            target_host: str,
            target_path: str,
            ssh_user: str = "root",
        ...
        if returncode != 0:
            logger.error(
                f"Failed to create target directory {target_path}: {stderr}")
            return False

        return True

    async def copy_compose_files(
            self,
            source_dir: str,
            target_host: str,
            target_dir: str,
        ...
        logger.info(
            f"Successfully copied compose files from {source_dir} to {target_host}:{target_dir}")
        return True
    ...
    async def update_compose_paths(
            self, compose_data: Dict, volume_mapping: Dict[str, str]) -> str:
        """Update compose data with new volume paths and return updated compose content"""
        try:
            # Create a deep copy of the compose data to avoid modifying the
            # original
            import copy
                    ...
                    if isinstance(volume, str):
                        ...
                        if ':' in volume:
                            ...
                            if len(parts) >= 2:
                                ...
                                new_host_path = volume_mapping.get(
                                    host_path, host_path)
            ...
            logger.info(
                f"Updated compose paths in {len(volume_mapping)} volume mappings")
            return updated_yaml
